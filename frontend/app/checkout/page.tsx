...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await refresh(); // clear cart state in UI
              ...
              <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Full Name"    error={errors.full_name}>
                  <input value={address.full_name}
                    ...
                    placeholder="Harshita Yadav" className={input(errors.full_name)} />
                </Field>
                <Field label="Phone Number" error={errors.phone}>
                  <input value={address.phone} maxLength={10}
                    onChange={e => setAddress(a => ({ ...a, phone: e.target.value }))}
                    placeholder="10-digit mobile number" className={input(errors.phone)} />
                </Field>
                <Field label="Address (House No, Street)" error={errors.address} className="sm:col-span-2">
                  <textarea value={address.address} rows={2}
                ...
                <Field label="City" error={errors.city}>
                  <input value={address.city}
                    ...
                    placeholder="e.g. Kota" className={input(errors.city)} />
                </Field>
                <Field label="State" error={errors.state}>
                  <input value={address.state}
                    onChange={e => setAddress(a => ({ ...a, state: e.target.value }))}
                    placeholder="e.g. Rajasthan" className={input(errors.state)} />
                </Field>
                <Field label="Pincode" error={errors.pincode}>
                  <input value={address.pincode} maxLength={6}
                    onChange={e => setAddress(a => ({ ...a, pincode: e.target.value }))}
                    placeholder="6-digit pincode" className={input(errors.pincode)} />
                </Field>
              </div>
            </div>
          ...
          <div className="w-full lg:w-80 lg:sticky lg:top-4 space-y-4">
            <div className="bg-white rounded-sm border border-gray-200">
                  ...
                  <div key={item.id} className="flex gap-3 px-4 py-3">
                    <div className="relative w-12 h-12 flex-shrink-0 bg-gray-50 rounded border border-gray-100">
                      {item.product.image_url
                        ? <Image src={item.product.image_url} alt={item.product.title}
                            fill className="object-contain p-0.5" sizes="48px" />
                        : <div className="w-full h-full bg-gray-100 rounded" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-700 line-clamp-2 leading-snug">{item.product.title}</p>
              ...
              <div className="p-4 space-y-2 border-t border-gray-100">
                <SummaryRow label={`Price (${cart.total_quantity} items)`}
              ...
              <div className="px-4 pb-4">
                <button onClick={handlePlaceOrder} disabled={placing}
                  className="w-full bg-[#FB641B] hover:bg-orange-600 disabled:bg-orange-300 text-white font-semibold py-3 rounded-sm text-sm transition-colors flex items-center justify-center gap-2">
                  {placing
...
function SummaryRow({ label, value, className = "", icon }: {
  label: string; value: string; className?: string; icon?: React.ReactNode;
}) {
  return (
