...
import Toast, { type ToastState } from "@/components/common/Toast";   // ← add
...
export default function OrderDetailPage() {
  ...
  const [toast,   setToast]   = useState<ToastState | null>(null);   // ← add

  useEffect(() => {
    orderApi.getOrder(orderNumber)
      .then((data) => {
        setOrder(data);
        // Show email toast only on fresh order success landing  // ← add
        if (isSuccess) {
          setToast({
            message: "Order confirmation email sent to your inbox!",
            variant: "success",
          });
        }
      })
    ...
    <div className="min-h-screen bg-[#F1F3F6] py-4">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="bg-white rounded-sm border border-gray-200 px-5 py-4 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <span
            className={`text-xs font-semibold px-3 py-1.5 rounded-full self-start sm:self-center ${ORDER_STATUS_COLOR[order.status]}`}
          >
      ...
      <Toast toast={toast} onClose={() => setToast(null)} />   {/* ← add */}
